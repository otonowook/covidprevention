...
기능적 요구사항
1. 시민이 보건소로 코로나검사를 신청한다V
2. 신청이 되면 신청내역이 검사소에 전달된다V
3. 검사소에서 검사를 받는다
5. 검사소는 검사결과를 보건소로 보내준다
6. 검사결과 음성일 경우 보건소는 증명서를 발급한다
7. 검사결과 양성일 경우 보건소는 시민을 격리소로 이송시킨다
8. 격리소에서는 약으로 시민을 치료한다.
10. 일주일이 지나면 시민을 집으로 이송시킨다
11. 검사소와 검사키트와 치료제가 부족한 경우 보건소에 발주를 요청한다.
12. 보건소에서는 검사키트와 치료제를 배송한다.
13. 배달담당은 약품을 배송한다.
...
# 분석/설계
 ![image](https://github.com/otonowook/covid-24/assets/152375871/070e834f-8314-4ecd-b110-9abcc8a0f145)
